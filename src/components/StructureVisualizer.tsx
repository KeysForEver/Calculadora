...
export const StructureVisualizer: React.FC<VisualizerProps> = ({ input }) => {
  ...
  // Generous SVG dimensions for maximum visual clarity without overlap
  const svgWidth = 640;
  const svgHeight = 240;
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-6">
        ...
        <div className="bg-slate-50 text-slate-800 rounded-xl p-4 border border-slate-200 shadow-sm">
          ...
          <div className="relative flex justify-center items-center bg-white rounded-xl p-4 border border-slate-200 shadow-inner overflow-x-auto">
            <svg width={svgWidth} height={svgHeight} className="max-w-full h-auto">
              ...
              <text
                ...
                y={Math.max(16, startY - 12)}
                fill="#334155"
                fontSize="11"
                fontWeight="bold"
                textAnchor="middle"
              >
                Largura Total: {largura.toFixed(2)} m ({linhasHorizontais} Linhas)
              </text>

              {/* Dimension Label Right */}
              <text
                x={startX + drawWidth + 20}
                ...
                transform={`rotate(90, ${startX + drawWidth + 20}, ${startY + drawHeight / 2})`}
              >
        ...
        <div className="bg-slate-50 text-slate-800 rounded-xl p-4 border border-slate-200 shadow-sm">
          <div className="flex items-center justify-between mb-3">
            ...
            <span className="text-[11px] font-medium text-amber-800 font-semibold">
                ...
                return (
                  <g key={`v-group-${j}`}>
                    ...
                    <g>
                      <rect
                        x={x - 12}
                        y={startY + drawHeight + (isStaggered ? 15 : 4)}
                        width="24"
                        height="11"
                        rx="3"
                        fill="#ffffff"
                      ...
                      <text
                        x={x}
                        y={startY + drawHeight + (isStaggered ? 23 : 12)}
                        fill="#78350f"
                        fontSize="7"
                        fontWeight="bold"
                        textAnchor="middle"
